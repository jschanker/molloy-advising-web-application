...
(function(namespace) {
	...
	var LASCodes = LASDB.find(dbnames.LAS_COURSE_COLLECTION_NAME, function(course) {
		return course._info.isLAS;
	}).map(function(course) {
	...
	*/
	...
	function getCourseDepth(courseAreaCode, courseNumber, startSemesterOffset, takenCourseCodes, maxDepth)
	{
		...
		courseItem.prerequisites.forEach(function(prerequisiteCourseCode) {
			...
			var neededSemesters = progressCalculator.getCourseDepth(new records.Course(prerequisiteCourseAreaCode, prerequisiteCourseNumber), 
	...
	***********/
	...
	function getMajorMinorWarning(majorMinorCourseRequirements, semestersToGraduate, courseHistoryCodes)
	{
		...
		for(var j = 0; j < 2; j++) {
			...
			for(var i = 0; i < majorMinorCourseRequirements.majorsRequirements[j].length; i++)
			{
				...
				var neededNumberOfSemesters = progressCalculator.getCourseDepth(new records.Course(area, code), OFFSET_FROM_FALL_2014, 
				...
				else if(progressCalculator.getCourseDepth(new records.Course(area, code), OFFSET_FROM_FALL_2014+1, 
	...
	namespace.exports.getCourseWarnings = function(courseInput)
	{
	...
	*/
	...
	namespace.exports.generateAdvice = function(courseInput)
	{
		var courseHistoryItems = parseCourseHistory(courseInput.courseHistory);
